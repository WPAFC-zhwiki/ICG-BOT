...
module.exports = function ( grunt ) {
	...
	grunt.task.registerTask( 'build', async function () {
		...
		async function doSpawn( cmd, args ) {
		...
		try {
			const { result, exitcode } = await doSpawn( 'pnpm', [ 'exec', 'tsc' ] );
	...
	grunt.task.registerTask( 'moveCompletedFiles', async function () {
		const done = this.async();
	...
	grunt.registerTask( 'default', [ 'clean:all', 'build', 'moveCompletedFiles', 'reloadFlag', 'clean:build' ] );
	grunt.registerTask( 'build:noReloadFlag', [ 'clean:all', 'build', 'moveCompletedFiles', 'clean:build' ] );
};
