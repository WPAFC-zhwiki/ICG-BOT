...
import { MessageHandler, Command, BaseEvents } from '@app/lib/handlers/MessageHandler.mjs';
import { inspect, getFriendlySize, getFriendlyLocation } from '@app/lib/util.mjs';
...
export type TelegramSendMessageOptions<
	E extends Convenience.ExtraPhoto | Convenience.ExtraReplyMessage = Convenience.ExtraReplyMessage
> = E & {
	withNick?: boolean
};
...
export class TelegramMessageHandler extends MessageHandler<TelegramEvents> {
	...
	#replyBuildTextAndOptions<E extends Record<string, unknown>>(
		...
		message: string,
		options?: Partial<E> & TelegramSendMessageOptions<Convenience.ExtraReplyMessage>
	) : [ text: string, options: Partial<E> & TelegramSendMessageOptions<Convenience.ExtraReplyMessage> ] | never {
		options = options || {};
	...
	public getFileLink( fileId: string ): Promise<import( 'url' ).URL> {
